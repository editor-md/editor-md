...
(function (global, factory) {
  ...
  var Lexer_1 = /*#__PURE__*/function () {
    ...
    _proto.blockTokens = function blockTokens(src, tokens, top, startline) {
      if (tokens === void 0) {
        tokens = [];
      }
      ...
      while (src) {
        ...
        if (token = this.tokenizer.blockquote(src)) {
          src = src.substring(token.raw.length);
          lined = token.raw.split('\n').length - 1;
          token.tokens = this.blockTokens(token.text, [], top, line);
          tokens.push(token);
          continue;
        } 
        ...
        if (token = this.tokenizer.list(src)) {
          ...
          for (i = 0; i < l; i++) {
            token.items[i].tokens = this.blockTokens(token.items[i].text, [], false, line);
          }
    ...
    _proto.parse = function parse(tokens, top) {
      ...
      for (i = 0; i < l; i++) {
        ...
        switch (token.type) {
          ...
          case 'heading':
            {
              ...
              out += this.renderer.heading(this.parseInline(token.tokens), token.depth, unescape$1(this.parseInline(token.tokens, this.textRenderer)), token.line);              
              continue;
            }
          ...
          case 'table':
            {
              ...
              out += this.renderer.table(header, body, token.line);
              continue;
            }
          ...
          case 'list':
            {
              ...
              out += this.renderer.list(body, ordered, token.line);
              continue;
            }
          ...
          case 'paragraph':
            {
              out += this.renderer.paragraph(this.parseInline(token.tokens), token.line);
              continue;
            }
